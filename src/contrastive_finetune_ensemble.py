...
class DocumentPairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.model_name == "rrivera1849/LUAR-MUD" or self.model_name == "/home/pezeng/rsp/LUAR-RU":
            # Remove token_type_ids from the tokenized output
...
class SiameseRoberta(nn.Module):
    def __init__(self, model):
        ...
        if model == "/home/pezeng/rsp/LUAR-RU":
            self.enc_model = AutoModel.from_pretrained(model, local_files_only=True, trust_remote_code=True)
        else:
...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(num_epochs), desc="Epoch Progress"):
        ...
        for batch in tqdm(train_loader, desc="Training"):
            ...
            if model_name == "rrivera1849/LUAR-MUD" or model_name == "/home/pezeng/rsp/LUAR-RU":
                with autocast():
    ...
    for i, row in test_df.iterrows():
        ...
        cosim = sklearn_cosine_similarity(vector1, vector2)
    ...
    for i, post in tqdm(enumerate(post1s), desc="Processing posts for embeddings", total=len(post1s)):
        ...
        neural_cosims.append(neural_cosim.item())    
